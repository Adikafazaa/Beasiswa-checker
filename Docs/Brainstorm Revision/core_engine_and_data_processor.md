...
> [!NOTE] 
> **[REVISION]** Definisi lengkap model dasar (`UserProfile`, `Scholarship`, `ScholarshipFlag`) dipindahkan ke `storage_layer_and_database_architecture.md`.
> Di bawah ini hanya menampilkan model spesifik untuk output evaluasi (`DimensionScores`, `EligibilityDetail`, `MatchResult`, `MatchReport`).
...
# ==========================================
# EVALUATION & REPORT OUTPUT MODELS
# ==========================================
...
class MatchResult(BaseModel):
    scholarship: 'Scholarship' # Direferensikan dari storage_layer
    is_eligible: bool
    ...
    user_flag: Optional['ScholarshipFlag'] = None # Direferensikan dari storage_layer
...
> [!NOTE]
> **[REVISION]** Skema DDL lengkap telah dipindahkan ke `storage_layer_and_database_architecture.md`. 
> Core Engine berinteraksi dengan database untuk membaca profile dan menyimpan hasil interaksi user (bookmark, flags).
...
**[REVISION] Memperbaiki BUG 1: Rentang IPK mutlak dihitung lebih dulu, lalu diberikan penalti terpisah jika kurang dari syarat minimum.**

* **Kalkulasi Dasar (Piecewise)**:
  * **Jika $\text{GPA} \ge 3.85$**: $100.0$
  * **Jika $3.50 \le \text{GPA} < 3.85$**: $85.0 + \left(\frac{\text{GPA} - 3.50}{0.35}\right) \times 15.0$
  * **Jika $3.00 \le \text{GPA} < 3.50$**: $70.0 + \left(\frac{\text{GPA} - 3.00}{0.50}\right) \times 15.0$
  * **Jika $2.50 \le \text{GPA} < 3.00$**: $55.0 + \left(\frac{\text{GPA} - 2.50}{0.50}\right) \times 15.0$
  * **Jika $\text{GPA} < 2.50$**: $\max\left(0.0, \frac{\text{GPA}}{2.50} \times 55.0\right)$
* **Penalti Batas Minimum (Jika $\text{min\_gpa} > 0.0$ dan $\text{GPA} < \text{min\_gpa}$)**:
  $$\text{Multiplier} = \max\left(0.3, \frac{\text{GPA}}{\text{min\_gpa}}\right)$$
  $$\text{Final Score} = \text{Score} \times \text{Multiplier}$$

#### 2. Skor Bahasa ($S_{\text{lang}} \in [0, 100]$):
**[REVISION] Memperbaiki BUG 3: Jika beasiswa tidak mensyaratkan skor bahasa, maka skor bahasa otomatis $100.0$.**

* **Jika Tidak Ada Syarat Bahasa ($\text{Req} = 0.0$)**: $100.0$
* **Jika Ada Syarat Bahasa**:
  $$\text{Surplus} = \text{User Best IELTS Equivalent} - \text{Min Req IELTS Equivalent}$$
  * **Jika $\text{Surplus} \ge 1.0$**: $100.0$
  * **Jika $0.0 \le \text{Surplus} < 1.0$**: $85.0 + (\text{Surplus} \times 15.0)$
  * **Jika $\text{Surplus} < 0.0$ (Di bawah syarat)**: $\max(0.0, 70.0 - (|\text{Surplus}| \times 25.0))$

#### 3. Skor Pengalaman Kerja ($S_{\text{exp}} \in [0, 100]$):
**[REVISION] Memperbaiki BUG 2: Jika beasiswa tidak mensyaratkan pengalaman kerja, skor bersifat netral (minimal $85.0$) atau mendapat bonus jika memiliki.**

* **Jika $\text{min\_exp} = 0.0$ (Tidak ada syarat)**:
  * **Jika $\text{Exp} \ge 4.0$**: $100.0$
  * **Jika $\text{Exp} \ge 2.0$**: $90.0$
  * **Jika $\text{Exp} > 0.0$**: $80.0 + \min(10.0, \text{Exp} \times 10.0)$
  * **Jika $\text{Exp} = 0.0$**: $85.0$
* **Jika $\text{min\_exp} > 0.0$**:
  * **Jika $\text{Exp} < \text{min\_exp}$**: $\left(\frac{\text{Exp}}{\max(1.0, \text{min\_exp})}\right) \times 60.0$
  * **Jika $\text{Exp} \ge (\text{min\_exp} + 2.0)$ atau $\text{Exp} \ge 4.0$**: $100.0$
  * **Jika $\text{min\_exp} \le \text{Exp} < (\text{min\_exp} + 2.0)$**: $80.0 + \min(20.0, (\text{Exp} - \text{min\_exp}) \times 10.0)$
...
class EligibilityFilter:
    ...
    def evaluate(user: UserProfile, scholarship: Scholarship) -> EligibilityDetail:
        ...
        # 4. Language Check (Explicit None-Safe) [REVISION] Memperbaiki BUG 4
        ...
        if not has_any_lang_req:
            ...
            lang_checks = []
            if scholarship.min_ielts is not None:
                if user.ielts_score is not None and user.ielts_score >= scholarship.min_ielts:
                    lang_checks.append(True)
                else:
                    lang_checks.append(False)
            if scholarship.min_toefl_ibt is not None:
                if user.toefl_ibt_score is not None and user.toefl_ibt_score >= scholarship.min_toefl_ibt:
                    lang_checks.append(True)
                else:
                    lang_checks.append(False)
            if scholarship.min_toefl_itp is not None:
                if user.toefl_itp_score is not None and user.toefl_itp_score >= scholarship.min_toefl_itp:
                    lang_checks.append(True)
                else:
                    lang_checks.append(False)
            if scholarship.min_duolingo is not None:
                if user.duolingo_score is not None and user.duolingo_score >= scholarship.min_duolingo:
                    lang_checks.append(True)
                else:
                    lang_checks.append(False)
            if scholarship.min_toeic is not None:
                if user.toeic_score is not None and user.toeic_score >= scholarship.min_toeic:
                    lang_checks.append(True)
                else:
                    lang_checks.append(False)

            if lang_checks and any(lang_checks):
                lang_passed = True
            else:
                # Fallback: normalized IELTS equivalent
                best_user = Normalizer.get_best_ielts_equivalent(user)
                min_req = Normalizer.get_scholarship_min_ielts(scholarship)
                lang_passed = best_user >= min_req and best_user > 0.0
...
class ScoringEngine:
    ...
    def calculate_dimensions(user: UserProfile, scholarship: Scholarship) -> DimensionScores:
        # 1. Academic Score (Strict Piecewise Order) [REVISION] Memperbaiki BUG 1: Dead-zone GPA dan penalti di bawah min_gpa dipisahkan.
        ...
        if gpa >= 3.85:
            ...
            acad = 85.0 + ((gpa - 3.50) / 0.35) * 15.0
        elif gpa >= 3.00:
            acad = 70.0 + ((gpa - 3.00) / 0.50) * 15.0
        elif gpa >= 2.50:
            acad = 55.0 + ((gpa - 2.50) / 0.50) * 15.0
        else:
            acad = max(0.0, (gpa / 2.50) * 55.0)

        # Apply penalty if below scholarship's minimum GPA requirement
        if min_gpa > 0.0 and gpa < min_gpa:
            penalty = max(0.3, gpa / min_gpa)  # Scale down to 30-100% of score
            acad = acad * penalty
            
        # 2. Language Score [REVISION] Memperbaiki BUG 3: Skor 100 jika tidak ada syarat bahasa.
        ...
        # 3. Work Experience Score [REVISION] Memperbaiki BUG 2: Tidak ada penalti jika tidak ada syarat pengalaman kerja.
        min_exp = scholarship.min_work_exp_years
        exp = user.work_exp_years
        
        if min_exp == 0.0:
            # No requirement: bonus for having experience, but no penalty for not having
            if exp >= 4.0:
                work = 100.0
            elif exp >= 2.0:
                work = 90.0
            elif exp > 0.0:
                work = 80.0 + min(10.0, exp * 10.0)
            else:
                work = 85.0  # Neutral: no requirement, no experience
        else:
            # Has requirement: original logic applies
            if exp < min_exp:
...
def sample_scholarships():
    return [
        ...
        Scholarship( # Beasiswa tanpa syarat min GPA, min IELTS, dan min EXP
            id="no-req-scholarship",
            name="No Requirement Scholarship",
            provider="Open Foundation",
            funding_type=FundingType.PARTIAL,
            target_degrees=[DegreeLevel.S1, DegreeLevel.S2],
            min_gpa=0.0,
            min_ielts=0.0,
            min_work_exp_years=0.0
        )
...
# [REVISION] Menambahkan test spesifik untuk mengecek perbaikan BUG (GPA, Exp, Lang Fallbacks)
def test_scoring_bug_fixes(sample_scholarships):
    scholarship = sample_scholarships[2] # no-req-scholarship
    user = UserProfile(
        user_id="test_user",
        name="Tester",
        age=20,
    ...
    # Test BUG 1 Fix: GPA 2.5 on min_gpa=0.0 should not get 100
    assert dim_scores.academic_score == 55.0, "GPA 2.5 harus dihitung berdasarkan rentang aslinya (55.0)"
    
    # Test BUG 2 Fix: Exp=0 on min_exp=0.0 should get neutral score (85.0)
    assert dim_scores.experience_score == 85.0, "Experience 0 dengan syarat 0 harus dapat nilai netral 85.0"
    
    # Test BUG 3 Fix: IELTS=0 on min_ielts=0.0 should get full score (100.0)
    assert dim_scores.language_score == 100.0, "Tanpa syarat bahasa, score bahasa harus 100.0"
...
1. **Langkah 1**: Install dependensi:
   ...
   ```
2. **Langkah 2**: Buat file `modules/models.py` (Data contract multi-user & flagging direferensikan dari skema `storage_layer`).
...
> **Catatan [REVISION]:** 
> - Perbaikan BUG logika perumusan skor (GPA, Experience, dan Bahasa) telah diimplementasikan dengan test case yang memastikan kalkulasi berjalan tepat sasaran.
